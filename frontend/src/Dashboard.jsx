...
function Toggle({ label, enabled, onChange }) {
  return (
    ...
    >
      ...
      <span className={`flex h-6 w-11 items-center rounded-full border p-1 transition ${enabled ? 'border-[#BBD987]/60 bg-[#BBD987]/25' : 'border-slate-600 bg-slate-700/50'}`}>
        <span className={`h-4 w-4 rounded-full transition ${enabled ? 'translate-x-5 bg-[#BBD987]' : 'translate-x-0 bg-slate-300'}`} />
      </span>
    </button>
  );
}
              ...
              <div className="flex flex-1 items-center justify-center">
                <div className="relative h-64 w-64">
                  <svg className="h-full w-full -rotate-90" viewBox="0 0 120 120">
                    ...
                    <circle cx="60" cy="60" r="42" fill="none" stroke="#ef4444" strokeWidth="18"
                      strokeDasharray={`${threatPercent} ${100 - threatPercent}`} pathLength="100" />
                    <circle cx="60" cy="60" r="42" fill="none" stroke="#f59e0b" strokeWidth="18"
                      strokeDasharray={`${warningPercent} ${100 - warningPercent}`}
                      strokeDashoffset={-threatPercent} pathLength="100" />
                    <circle cx="60" cy="60" r="42" fill="none" stroke="#BBD987" strokeWidth="18"
                      strokeDasharray={`${100 - threatPercent - warningPercent} ${threatPercent + warningPercent}`}
                      strokeDashoffset={-(threatPercent + warningPercent)} pathLength="100" />
                  </svg>
          ...
          <div className="grid gap-4 rounded-lg border border-white/10 bg-[#2F3E46]/40 p-5 shadow-2xl shadow-black/20">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="rounded-lg border border-red-500/20 bg-red-500/10 p-5">
                <p className="text-sm font-semibold text-red-100">Total Threats</p>
        ...
        <section className="grid gap-5 lg:grid-cols-[0.85fr_1.15fr]">
          <div className="rounded-lg border border-white/10 bg-[#2F3E46]/40 p-5 shadow-2xl shadow-black/20">
            <div className="flex items-center justify-between gap-4">
              ...
              <span className="rounded-full border border-[#BBD987]/20 bg-[#BBD987]/10 px-3 py-1 text-xs font-bold text-[#BBD987]">Guarded</span>
            </div>
            <div className="mt-6 rounded-lg border border-white/10 bg-[#0B1B3D]/60 p-4">
              <div className="flex items-center justify-between gap-4">
                <label htmlFor="threshold" className="text-sm font-medium text-slate-100">Threat Score Threshold</label>
                <span className="text-lg font-black text-[#BBD987]">{threshold}%</span>
              </div>
              <input id="threshold" type="range" min="1" max="100" value={threshold}
          ...
          <div className="rounded-lg border border-white/10 bg-[#2F3E46]/40 p-5 shadow-2xl shadow-black/20">
            ...
            <div className="mt-5 max-h-[23rem] overflow-y-auto rounded-lg border border-white/10 bg-black/30 p-3 font-mono text-sm">
              <div className="grid min-w-[38rem] gap-3">
                ...
                {alerts.map((alert) => {
                  const styles = getAlertStyles(alert.verdict);
                  ...
                  return (
                    <div key={alert.id}>
                      ...
                      >
                        ...
                        <span className={`rounded-full border px-2 py-1 text-center text-xs font-black uppercase ${styles.badge}`}>{alert.verdict}</span>
